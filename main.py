...
def compute_hybrid_score(parsed, job_description, academic=None,
    ...
    if not set_skills:
        ...
        # safer: overlap ratio = inter / len(set_skill  s)
...
async def job_matching(
    ...
    for academic in students:
        ...
        # FILTER: CGPA
        # -------------------------------
        cgpa = float(academic.get("cgpa", 0))
        cgpa_norm = cgpa * 10 if cgpa <= 10 else cgpa
        min_cgpa_norm = min_cgpa * 10 if min_cgpa <= 10 else min_cgpa

        if cgpa_norm < min_cgpa_norm:
            continue

        # -------------------------------
        # FILTER: SSC
        # -------------------------------
        ssc_val = academic.get("xPercentage", "0").replace("%", "")
        ssc = float(ssc_val) if ssc_val else 0
        if min_ssc > 0 and ssc < min_ssc:
            continue

        # -------------------------------
        # FILTER: HSC
        # -------------------------------
        hsc_val = academic.get("xiiPercentage", "0").replace("%", "")
        hsc = float(hsc_val) if hsc_val else 0
        if min_hsc > 0 and hsc < min_hsc:
            continue
        ...
        student_skills = [s.strip().lower() for s in other.get("skills", "").split(",")]
        ...
        # -------------------------------
        text_pool = " ".join([
            personal.get("profile_summary", ""),
            academic.get("project", ""),
            academic.get("internship", ""),
            other.get("achievements", ""),
            other.get("certifications", "")
        ]).lower()

        desc_words = job_description.lower().split()
        score = sum(1 for w in desc_words if w in text_pool)

        # normalize score into %
        max_score = len(desc_words) if len(desc_words) > 0 else 1
    ...
    # sort by score
    shortlisted = sorted(shortlisted, key=lambda x: x["match_percentage"], reverse=True)
